...
export async function generateMetadata({ params }: Props) {
  ...
  if (!monitor) return { title: "Status | ZeroPulse" };
  return {
    title:       `${monitor.name} — Status`,
    description: `Live uptime status for ${monitor.url}`,
  };
}
  ...
  return (
    <div className="min-h-screen grid-bg flex flex-col">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-12 space-y-6">
        {/* Status hero */}
        <div className={`card p-8 text-center border ${
          isUp ? "border-pulse-green/30" : "border-pulse-red/30"
        }`}>
        ...
        <div className="card p-6">
          <h2 className="text-sm font-semibold text-pulse-text mb-4">
            Recent checks
          </h2>
          {recentChecks.length === 0 ? (
            <p className="text-pulse-dim text-sm text-center py-6 font-mono">
            ...
            <div className="space-y-2">
              {recentChecks.map((check, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between py-2 px-3 rounded-lg bg-pulse-muted text-xs font-mono"
                >
                  <div className="flex items-center gap-2">
                    {check.isUp ? (
                    ...
                    {check.statusCode && (
                      <span className="text-pulse-dim">HTTP {check.statusCode}</span>
                    )}
                  </div>
        ...
        <p className="text-center text-xs text-pulse-dim font-mono">
          Powered by{" "}
          <a href="/" className="text-pulse-cyan hover:underline">
            ZeroPulse
          </a>{" "}
          &amp;{" "}
          <a
...
function formatResponse(ms: number | null) {
  if (ms === null) return "—";
  return `${ms}ms`;
}
